#!/usr/bin/env python3

from functools import partial

import numpy as np
import rclpy
from rclpy.node import Node
from rclpy.parameter import Parameter
from rclpy.wait_for_message import wait_for_message
from numpy.linalg import norm

from tf2_ros import Buffer, TransformListener
from geometry_msgs.msg import Twist
from std_msgs.msg import Bool, String
from dwm1001_ros_interfaces.msg import UWBMeas

from follow_me.utils import (
    timer_shutdown,
    PID,
    RateLimiter,
    get_angle,
    get_transform,
)


class PurePursuit(Node):
    def __init__(self):
        super().__init__("pure_pursuit")

        self.declare_parameter("base_frame", Parameter.Type.STRING)
        self.declare_parameter("odom_frame", Parameter.Type.STRING)
        self.declare_parameter("cmd_topic", Parameter.Type.STRING)
        self.declare_parameter("follow_frame", Parameter.Type.STRING)
        self.declare_parameter("follow_distance", Parameter.Type.DOUBLE)
        self.declare_parameter("prediction_frequency", Parameter.Type.DOUBLE)
        self.declare_parameter("safe_distance", Parameter.Type.DOUBLE)
        self.declare_parameter("allow_backwards_motion", Parameter.Type.BOOL)
        # values mirrored from the /uwb/twr node parameters (supplied via launch)
        self.declare_parameter("twr_ids", Parameter.Type.STRING_ARRAY)
        self.declare_parameter("twr_positions", Parameter.Type.DOUBLE_ARRAY)

        self.base_frame = self.get_parameter("base_frame").value
        self.odom_frame = self.get_parameter("odom_frame").value
        self.cmd_topic = self.get_parameter("cmd_topic").value
        self.pos_frame = self.get_parameter("follow_frame").value
        self.ready_topic = "/detection_ready"

        self.follow_distance = self.get_parameter("follow_distance").value
        self.detect_frequency = self.get_parameter("prediction_frequency").value
        self.safe_distance = self.get_parameter("safe_distance").value
        self.movement = [0.0, 0.0]
        self.allow_backwards = self.get_parameter("allow_backwards_motion").value
        self.backwards = False
        self.control_per = 0.05
        self.switch_count = 0
        self.linear_control = PID(
            0.7, 0.1, 0.5, self.control_per, 0.1, low=0.0, high=0.5
        )
        self.angular_control = PID(
            0.6, 0.005, 1.0, self.control_per, 0.1, low=-0.2, high=0.2
        )
        self.linear_limit = RateLimiter(0.9, -0.9, self.control_per, self.get_clock())
        self.angular_limit = RateLimiter(1.0, -1.0, self.control_per, self.get_clock())

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self, spin_thread=True)

        self.ids = self.get_parameter("twr_ids").value
        self.coords = self.get_parameter("twr_positions").value
        self.positions = {}
        self.subscribers = {}
        self.ranges = {}
        for i in range(len(self.ids)):
            id = self.ids[i]
            self.positions[id] = np.array(
                [
                    [self.coords[3 * i]],
                    [self.coords[3 * i + 1]],
                    [self.coords[3 * i + 2]],
                ]
            )
            self.ranges[id] = np.nan
            topic = "/uwb/twr/ID_" + id + "/distances"
            self.subscribers[id] = self.create_subscription(
                UWBMeas, topic, partial(self.d_callback, id=id), 1
            )

        self.cmd_vel_pub = self.create_publisher(Twist, self.cmd_topic, 1)
        self.cmd_timer = self.create_timer(0.02, self.move)
        self.fol_timer = None

        self.sound_pub = self.create_publisher(String, "/log_sound", 1)

    def d_callback(self, msg, id):
        if len(msg.measurements) != 0:
            self.ranges[id] = msg.measurements[0].dist

    def move(self):
        """periodically publish to the command velocity topic"""

        tw = Twist()
        tw.linear.y = 0.0
        tw.linear.z = 0.0
        tw.angular.x = 0.0
        tw.angular.y = 0.0

        all_ready = True
        min_range = np.inf
        for id in self.ids:
            if np.isnan(self.ranges[id]):
                all_ready = False
                break
            min_range = np.min([min_range, self.ranges[id]])
        if all_ready and min_range > self.safe_distance:
            tw.linear.x = float(np.clip(self.movement[0], -0.5, 1.2))
            tw.angular.z = float(np.clip(self.movement[1], -0.8, 0.8))
        else:
            s = String(data="You are too close, stopping movement")
            self.sound_pub.publish(s)
            self.linear_control.reset()
            self.angular_control.reset()
        self.cmd_vel_pub.publish(tw)

    def follow(self):
        t = get_transform(self, self.tf_buffer, self.base_frame, self.pos_frame)
        if t is None:
            return None

        o = np.array([[t[0, 3]], [t[1, 3]]])
        d = norm(o)

        # compute necessary steering inputs
        Va = np.array([[(-1) ** self.backwards], [0], [0]])
        Vb = np.concatenate((o, np.array([[0]])))
        angle = get_angle(Va, Vb, np.array([[0], [0], [1]]))

        # switch between driving forwards and backwards
        if self.allow_backwards and np.abs(angle) > np.deg2rad(150):
            self.switch_count += 1
            if self.switch_count == 5:
                # switch only if the human wants to do it (suppress possible oscillations of the localisation)
                self.switch_count = 0
                self.get_logger().warn("SWITCHING DIRECTION OF MOVEMENT")
                s = String(data="Switching direction of movement")
                self.sound_pub.publish(s)
                self.angular_control.reset()
                self.backwards = not self.backwards
                Va = np.array([[(-1) ** self.backwards], [0], [0]])
                angle = get_angle(Va, Vb, np.array([[0], [0], [1]]))

        # compute error of distance to the leader
        error = d - self.follow_distance

        linear = (-1) ** self.backwards * self.linear_control.control(error)
        angle_err = angle
        angular = self.angular_control.control(angle_err)

        # do not move away from the human to increase the gap
        linear = np.clip(linear, (-1000) * self.backwards, 1000 * (not self.backwards))

        self.movement = [
            self.linear_limit.limit(linear),
            self.angular_limit.limit(angular),
        ]

    def run(self):
        """wait for detection and then start the operation (start timer and spin)"""
        ok, _ = wait_for_message(Bool, self, self.ready_topic, time_to_wait=10.0)
        if not ok:
            self.get_logger().error("Human detection is unavailable")
            s = String(data="Human detection is unavailable")
            self.sound_pub.publish(s)
            return
        s = String(data="I am ready to follow you")
        self.sound_pub.publish(s)
        self.fol_timer = self.create_timer(self.control_per, self.follow)
        rclpy.spin(self)

    def shutdown_hook(self):
        timer_shutdown(self.fol_timer)
        self.movement = [0.0, 0.0]
        timer_shutdown(self.cmd_timer)


def main(args=None):
    rclpy.init(args=args)
    node = PurePursuit()
    try:
        node.run()
    except KeyboardInterrupt:
        pass
    finally:
        node.shutdown_hook()
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()


if __name__ == "__main__":
    main()
